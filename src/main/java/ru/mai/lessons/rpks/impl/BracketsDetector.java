...
public class BracketsDetector implements IBracketsDetector {
    ...
    public void actionWithBrackets(List<Integer> result, int i, Map<String, String> brackets, String currentElement,
        ...
        } else if (isOpenOrBoth == 2) {
            stackBrackets.push(currentElement);
            stackPositions.push(i + 1);
        } else if (isOpenOrBoth == 3) {
    ...
    public List<Integer> findBracketsInListProcess(String line, Map<String, String> brackets) {
        ...
        for (int i = 0; i < line.length(); ++i) {
            ...
            if (brackets.containsValue(currentElement) && brackets.containsKey(currentElement)) {
                actionWithBrackets(result, i, brackets, currentElement, stackBrackets, stackPositions, 1);
            } else if (brackets.containsKey(currentElement)) {
                actionWithBrackets(result, i, brackets, currentElement, stackBrackets, stackPositions, 2);
            } else if (brackets.containsValue(currentElement)) {
                actionWithBrackets(result, i, brackets, currentElement, stackBrackets, stackPositions, 3);
            }
        }
